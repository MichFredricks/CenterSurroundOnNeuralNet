function TrainingAccuracy = neuralnetwork(X,y,receptive_field,hidden_layer_size,center_on)
%%
...
if receptive_field
    if center_on
        load('center_on_receptive_field_1d.mat');
    else
        load('center_off_receptive_field_1d.mat');
    end
    for i = 1:size(X,1)
        temp = [0 0 0 X(i,:) 0 0 0];
        for j = 1:size(X,2)
            out = sum(sum(temp(j:j+6).*weights));
            X(i,j) = out;
        end
    end
end
